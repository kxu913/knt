...
public class Env implements ApplicationContextAware, DisposableBean {
	...
	// Support annotation, which @Service
	...
	public void setApplicationContext(ApplicationContext applicationContext) {
		...
		if (Env.applicationContext != null) {
			logger.warn("SpringContextHolder中的ApplicationContext被覆盖, 原有ApplicationContext为:"
					+ Env.applicationContext);
		}

		Env.applicationContext = applicationContext; // NOSONAR
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> T getUserDefinedBean(String packageName, String className) {
		String beanName = className + BEAN_POSTFIX;
		ConfigurableApplicationContext applicationContext = (ConfigurableApplicationContext) Env
				.getApplicationContext();
		DefaultListableBeanFactory beanFactory = (DefaultListableBeanFactory) applicationContext
				.getBeanFactory();
		if (!beanFactory.containsBean(beanName)) {
			BeanDefinitionBuilder bdb = BeanDefinitionBuilder
					.rootBeanDefinition(packageName + "." + className);
