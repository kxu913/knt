...
public class ProductService {
	...
	/*
	 * public Product getProduct(long productId){ return productMapper.getProduct(productId); }
	 */
	...
	public List<List<Product>> getProductList4Front(int categoryId, int numInLine) {
		...
		if (productList != null && productList.size() > 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				if ((i + 1) == size) {
					/*
					 * if(temp.size()<numInLine && temp.size()>0){ while(temp.size()<numInLine){ temp.add(null); } }
					 */
					resultList.add(temp);
				}
			}
	...
	/*
	 * public List<Product> getProductPageList(long categoryId, int start, int end){ return productMapper.getProductPageList(categoryId,
	 * start, end); }
	 * 
	 * public List<Product> getRecommendProductList() { return productMapper.getRecommendProductList(); }
	 * 
